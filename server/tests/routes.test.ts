...
// //   test('Should cancel interview', (done) => {
// // eslint-disable-next-line max-len
// //     request(app).patch('/api/user/627d1d11f5243856362e8a8c').set('Cookie', [`token=${process.env.TEST_TOKEN}`])
// //       .end((err, res) => {
// //         if (err) {
// //           return done(err);
// //         }
...
// //   });
// //   test('Should throw an error if user not logged in', (done) => {
// //     request(app).patch('/api/user/627d1d11f5243856362e8a8c').expect(401).end((err, res) => {
// //       if (err) {
// //         return done(err);
// //       }
// //       expect(res.body.message).toBe('Login First!');
